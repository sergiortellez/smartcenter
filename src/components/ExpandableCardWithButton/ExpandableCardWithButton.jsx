...
The 'cover' or card is always visible, it has a button to trigger the animation and display of the card content. 
It renders two articles, one for the cover 
and one for the expandedContent. It's meant to be used in a grid layout using 
grid-template-areas. But if no area is defined it can be used just as a normal grid element. 
The  number of rows and columns can be specified by the dev 
in the area template in the css. Also the space where the expanded content will 
be displayed. 
     
* Parameters:
    - cardContent (JSX): The content that will be displayed in the button part 
      of the card.
    ...
    - buttonStyles (styles object): Additional classes for styling on the button.
    - expandedStyles (styles object): Additional classes for styling on the expanded content.
    - buttonText (string or JSX): The text that will be displayed in the button.
...
//styles
import styles from './ExpandableCardWithButton.module.css'
...
export default function ExpandableCard({
    ...
    return (
        <>
            {/* button part */}
            <article
                className={`${styles.cardBase} ${styles.card} ${className} ${isOpen ? styles.openedCard : ''}`}
                style={collapsedGridStyles || {}}

                aria-expanded={isOpen}
                aria-label="Open expanded content"
            >
                {/* content for the card cover */}
                {cardContent}

                <button onClick={handleToggle} className={buttonStyles ? buttonStyles : ''}>{buttonText}</button>
